  ...
  return (
    <aside style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '28px' }}>
        <div style={{ fontSize: '1.375rem', fontWeight: 800, color: '#16a34a', letterSpacing: '-0.02em' }}>
          💰 PRÉSTAMOS
        </div>
        <button
          onClick={toggleDark}
          aria-label="Alternar modo oscuro"
          style={{ width: '24px', height: '24px', borderRadius: '8px', border: '0', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', color: 'var(--text-nav)', transition: 'all 0.2s', padding: '0', outline: 'none' }}
          onMouseEnter={e => { e.currentTarget.style.background = darkMode ? '#1a3a2a' : '#f0fdf4'; e.currentTarget.style.color = '#16a34a'; }}
          onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--text-nav)'; }}
        >
          {darkMode ? (
